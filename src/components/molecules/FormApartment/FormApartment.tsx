...
export const FormApartment: React.FunctionComponent<FormApartmentProps> = (
  props
) => {
  ...
  const addOrUpdateApartment = async (e: SyntheticEvent) => {
    ...
    try {
      const res = await fetch(
        `${prefix}/api/apartment/${id ? "update" : "add"}`,
        {
          method: "POST",
          headers: {
  ...
  if (added) {
    return (
      <FormApartmentStyle>Apartament dodany lub zmieniony</FormApartmentStyle>
    );
  }
  ...
  return (
    <FormApartmentStyle>
      ...
      {id && (
        <Button className="deleteBtn" onClick={() => deleteApartment(id)}>
          Usuń
        </Button>
      )}
    </FormApartmentStyle>
